  ...
  return (
    <footer className="mt-8 border-t border-border/70">
      <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-8 sm:flex-row sm:items-center sm:justify-between sm:px-6">
        <Link href="/" prefetch={false} className="flex items-center gap-2 text-sm font-medium">
        ...
        <nav aria-label="Pied de page" className="flex flex-wrap gap-x-5 gap-y-2 text-sm text-muted-foreground">
          {LINKS.map((l) => (
            <Link key={l.href} href={l.href} prefetch={false} className="transition-colors hover:text-foreground">
              {l.label}
            </Link>
          ))}
