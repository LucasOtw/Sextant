    ...
    <ul className={cn("grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4", className)}>
      {themes.map((t) => (
        <li key={t.slug}>
          <Link
            href={`/theme/${t.slug}`}
            className="group flex h-full flex-col gap-2 rounded-xl bg-card p-4 ring-1 ring-foreground/10 transition-all hover:-translate-y-0.5 hover:shadow-md hover:ring-foreground/20"
          >
            <span className={cn("size-2.5 rounded-full", t.tone)} aria-hidden />
