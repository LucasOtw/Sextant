 ...
 * partagé par tous les cœurs, la liste /favoris et le compteur du header.
 */
export function FavoritesProvider({ userId, children }: Props) {
  ...
  }, [userId, refresh, consumePending, applyIds]);
  ...
  const toggle = useCallback<FavoritesContext["toggle"]>(
    async (snapshot) => {
      ...
      try {
        if (wasFavorite) {
          ...
          toast("Retiré de vos favoris.", { action: { label: "Annuler", onClick: () => void toggleRef.current?.(snapshot) } });
          return "removed";
        }
    ...
    [userId, ready, refresh, router, added, applyIds],
  );
