...
export function WorkCard({ work, variant = "list" }: Props) {
  ...
  return (
    <article
      className={cn(
        "group relative flex flex-col gap-2 rounded-xl bg-card p-4 ring-1 ring-foreground/10 transition-shadow hover:shadow-md hover:ring-foreground/20",
    ...
    >
      ...
      <h3 className={cn("title-serif leading-snug", compact ? "text-lg" : "text-xl sm:text-2xl")}>
      ...
      {!compact && abstract && (
        <p className="text-sm leading-relaxed text-foreground/80">{truncateWords(abstract, 45)}</p>
      )}
